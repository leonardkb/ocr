...
function App() {
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col items-center justify-center p-4">
      ...
      <div className="py-10 sm:py-10">
        ...
        <h4 className="text-base sm:text-lg md:text-xl lg:text-xl xl:text-xl font-semibold text-center text-gray-700">
  Markdown your bills, documents faster and easier within a short span of time
      ...
      {markdown && (
        <div className="mt-6 w-full sm:max-w-3xl md:max-w-4xl lg:max-w-5xl bg-white p-4 rounded shadow">
        <h2 className="text-xl font-semibold mb-2">Markdown Output:</h2>
        <pre className="text-sm bg-gray-100 p-2 rounded overflow-auto">
          {/* Split markdown by lines and wrap each in a span */}
          {markdown.split("\n").map((line, index) => (
            <span
              key={index}
              className="block p-2 transition-transform transform hover:scale-105 hover:bg-blue-100 rounded cursor-pointer"
            >
              {line}
            </span>
          ))}
